...
export function PedidosClient({ initialProducts, categories }: PedidosClientProps) {
    const { data: session } = useSession();
    const router = useRouter();

    // Global Cart Store
    const { items: cart, addItem, removeItem, updateQuantity, clearCart, totalBRL } = useCartStore();
    ...
    const calculateTotals = () => {
        const totalBRLValue = totalBRL();
        const rate = parseFloat(exchangeRate) || 0;
        const totalPYG = totalBRLValue * rate;
        return { totalBRL: totalBRLValue, totalPYG };
    };
    ...
    const handleCreateOrder = async () => {
        ...
        try {
            ...
            const { totalBRL: totalBRLValue, totalPYG } = calculateTotals();
            ...
            toast.success(`Pedido ${result.order.order_number} criado!`);
            ...
            setCustomerPhone("");
            setIsCartOpen(false);
            router.refresh(); // Refresh server data (e.g. stock)
    ...
    const { totalBRL: currentTotalBRL, totalPYG: currentTotalPYG } = calculateTotals();
                        ...
                        <div className="border-t bg-background p-4 space-y-3">
                            <div className="space-y-2">
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="font-semibold">{formatBRL(currentTotalBRL)}</span>
                                </div>
                                <div className="flex justify-between text-xs text-muted-foreground">
                                    <span>Guarani</span>
                                    <span>{formatPYG(currentTotalBRL)}</span>
                                </div>
                            </div>
                            <Button
                                ...
                                <div className="w-full space-y-4">
                                    <div className="space-y-1">
                                        <div className="flex justify-between font-bold text-lg">
                                            <span>Total:</span>
                                            <span>{formatBRL(currentTotalBRL)}</span>
                                        </div>
